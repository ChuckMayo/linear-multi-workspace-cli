...
const ALLOWED_FIELDS: Record<EntityName, ReadonlySet<string>> = {
  issue: new Set([
    ...
    'dueDate',
    'snoozedUntilAt',
    // Phase 2 PLAN 02-01: `snippet` is projected from
    // `IssueSearchResult.metadata.snippet` by the issue-search runtime
    // (Plan 02-05). Adding it here lets `--fields=snippet` validate
    // without INVALID_FIELD; runtimes that don't surface a snippet leave
    // the leaf as `null`.
    'snippet',
    'state.id',
    'state.name',
